class AvailabilityCalendar < ApplicationRecord
  ...
  def refresh!(work_schedules: nil)
    # 1. Fetch data
    schedules = work_schedules || WorkSchedule.where(is_active: true)
    appointments = Appointment.where(scheduled_at: self.period_start..self.period_end)

    # 2. Generate slots
    generator = SlotGenerator.new(schedules, appointments)
